...
function MainApp() {
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      setStatus('analyzing');
      setStatusMessage('LLM is extracting logic & UI context...');

      const prompt = `Analyze these source code files.
      Extract core architecture: classes, functions, caller-callee relationships.
      ...
      // --- SECURE VERCEL API CALL WITH USER ID ---
  ...
  const handleRefactor = async (node) => {
    ...
    try {
      ...
      setRefactorSuggestion("Failed to generate suggestion. Check your API keys.");
  ...
  const exportMap = (type) => {
    ...
    img.onerror = () => { 
      showToast("PNG generation failed. Falling back to SVG.", 'warning');
      triggerDownload(svgUrl, 'architecture.svg'); 
    };
    img.src = svgUrl;
  };
      ...
      <header className="flex items-center justify-between px-6 py-4 bg-white border-b border-slate-200 z-10 shrink-0">
        ...
        <div className="flex items-center space-x-2">
          ...
          <a href={TIP_JAR_LINK} target="_blank" rel="noopener noreferrer" className="flex items-center space-x-2 px-4 py-2 text-pink-600 bg-pink-50 hover:bg-pink-100 rounded-full font-bold text-sm transition-colors border border-pink-200 shadow-sm">
            <Heart size={16} className="fill-red-500 text-red-500" /> <span className="hidden sm:inline">Support</span>
          </a>
